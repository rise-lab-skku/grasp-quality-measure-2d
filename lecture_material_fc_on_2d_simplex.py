...
class Tableau:
    ...
    def find_pivot(self) -> Tuple[Any, Any]:
        ...
        col_idx = np.argmax(self.tableau[0, :-1])
    ...
    def pivot(self, row_idx: int, col_idx: int) -> np.ndarray:
        ...
        # clip to zero values below tolerance
        self.tableau = np.where(np.abs(self.tableau) < 1e-10, 0, self.tableau)
        return self.tableau
    ...
    def run_simplex(self) -> Dict[Any, Any]:
        ...
        for _ in range(Tableau.maxiter):
            ...
            if self.n_artificial_vars == 0 and self.stop_iter:
...
if __name__ == "__main__":
    # Non FC grasp example
    print('===== Non FC grasp example ====')
    A = np.array(
        [
            [  -5,   5, -19, -29, 0, 0,  48],
            [  -1,  -1,  -1,  -1, 0, 0,   0],
    ...
    print(Tableau(A, 4, 2).run_simplex(), '\n\n')

    # FC grasp example
    print('===== FC grasp example ====')
    A = np.array(
        [
            [  -5,   5, -19, -29,   5,  -5, 0, 0,  48],
            [  -1,  -1,  -1,  -1,  -1,  -1, 0, 0,   0],
            [ -17,  -7,  -7, -17,  17,   7, 1, 0,  24],
            [  12,  12, -12, -12, -12, -12, 0, 1,  24],
        ],
        dtype="float64",
    )
    print(Tableau(A, 6, 2).run_simplex())
